let elements = []
let prioritize_elements;
let priority_button = document.getElementsByClassName("button_priority");
...
const create_item = function() {
  ...
  if (input === "") {
      ...
      elements[index].htmlPriorityButton.setAttribute("class", "button_priority");
  ...
  for (let z = 0; z < priority_button.length; z++) {
    priority_button[z].onclick = function () {
      if (elements[z].priority == false) {
        ...
        prioritize = true;
        priority_button[z].style.backgroundColor = "red";
        ...
        row[elements.length - 1].after(prioritize_elements);
